...
for index, row in df.iterrows():
    ...
    data = Data(user_id, business_id, timeStamp, float(row["stars"]))
    dataset.append(data)
...
for index, row in df.iterrows():
    ...
    business_id = business_dict[business]
    timeStamp = time.mktime(datetime.strptime(date, "%Y/%m/%d %H:%M").timetuple())/(30*24*3600)
    sim_userset = userset[user_id].sim_userset
    data = Data(user_id, business_id, timeStamp, 0)
    '''
    处理冷启动问题：
    ...
    if scoring_matrix[user_id][business_id] > 1e-6:
        ...
        for sim_user in sim_userset:
            if(scoring_matrix[sim_user][business_id] > 1e-6):
                up += sim_matrix[user_id][sim_user]*(scoring_matrix[sim_user][business_id]-userset[sim_user].avg_star)*(1/(1+alpha*math.fabs(data.timeStamp-time_matrix[sim_user][business_id])))
    ...
    result.append([user, business, date, "{:.2f}".format(preq)])

f = open('test1.csv', 'w', newline='')
writer = csv.writer(f)
writer.writerow(['user_id', 'business_id', 'date', 'star'])
